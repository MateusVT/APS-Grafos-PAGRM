...
public class BuscaProfundidade {
    ...
    public void BuscaProfundidadeRemoveCiclo(Grafo grafo, Vertice vertice, String anterior) {
        ...
        for (Aresta aresta : grafo.getListaAdjacencia().get(vertice)) {
            if (!aresta.getDestino().getId().equals(anterior)) {
                Vertice vert = aresta.getDestino();

                if (vert.getCor().equals("Cinza")) {
                    grafo.setCiclico(true);
                    aresta.setEmCiclo(true);
                } else if (vert.getCor().equals("Branco")) {
                    vert.setPi(vertice);
                    BuscaProfundidadeRemoveCiclo(grafo, vert, vertice.getId());
                }
            }
        }
